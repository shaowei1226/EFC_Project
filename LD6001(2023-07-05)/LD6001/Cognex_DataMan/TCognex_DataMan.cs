...
namespace Cognex.DataMan
{
    public class TReader_Cognex_DataMan
    {
        ...
        public string    Log_Source = "TReader_Cognex_DataMan";
        ...
        public void Init()
        {
            if (!Inig_OK)
            {
                ...
                {
                    Log_Add("Init", ex.Message, emLog_Type.Error);
                }
            }
        }
        ...
        public bool Get_Code(ref string read_code)
        {
            ...
            if (Connect && !On_Life)
            {
                ...
                if (read_code != "R00" && read_code != "")
                {
                    Log_Add(fun, string.Format("Get_Code = {0:s}", Read_String));
                    result = true;
                }
                else
                {
                    Log_Add(fun, string.Format("Get_Code = {0:s}", Read_String), emLog_Type.Warning);
                    result = false;
                }
            }
            else
