...
namespace EFC.Printer.Zebra
{
    ...
    public class TZebra_Printer : TBase_SerialPort
    {
        ...
        public string Log_Source = "TZebra_Printer";
        ...
        {
            bool result = false;
            string fun = "Set_Label_Top";
            string cmd = "";

            cmd = "^XA" + "^LT" + number.ToString() + "^XZ";
            result = Write_String(cmd);

            if (!result) Log_Add(fun, "[TZebra_Printer] Set_Label_Top 失敗", emLog_Type.Error);
        ...
        public bool Set_Tear_Off(int number)
        {
            ...
            if (!result) Log_Add(fun, "[TZebra_Printer] Set_Tear_Off 失敗", emLog_Type.Error);
            return result;
        }
        ...
        public bool Start()
        {
            ...
            if (!result) Log_Add(fun, "[TZebra_Printer] 印表機啟動失敗", emLog_Type.Error);
    ...
    public class TZebra_Value_List : TBase_Class
    {
        ...
        {
            int no = Get_Index(name);
            if (no >= 0)
            {
                Items[no].Value = value;
                Reflash = true;
            }
        }
